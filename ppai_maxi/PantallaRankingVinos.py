...
class PantallaRankingVinos:
    ...
    def confirmacion_generacion_informe(self,eleccion,tipo_visualizacion):
        ...
        if eleccion == "Si":
            ...
            if confirmacion and tipo_visualizacion == "Excel":
                ...
                res = self.pedirConfirmacionExportacion()
                if res :
                    self.fin_cus = tk.Label(self.root, text="Se genero el informe en excel")
                    self.fin_cus.place(relx=0.5, rely=0.55, anchor="center")
            elif confirmacion and tipo_visualizacion == "Pdf":
                self.lbl_pregunta.destroy()
                self.lbl_preguntapositiva.destroy()
                self.lbl_preguntanegativa.destroy()
                self.fin_cus = tk.Label(self.root, text="Se genero el informe en Pdf")
                self.fin_cus.place(relx=0.5, rely=0.55, anchor="center")
            else:
                self.lbl_pregunta.destroy()
                self.lbl_preguntapositiva.destroy()
    ...
    def pedirConfirmacionExportacion(self):
        ...
        self.lbl_preguntapositiva_export = tk.Button(self.root, text="Generar exportacion", command=lambda:self.tomarConfirmacionExportacion("Si"))
        self.lbl_preguntapositiva_export.place(relx=0.35, rely=0.65, anchor="center", y=27)
        self.lbl_preguntanegativa_export = tk.Button(self.root, text="No generar exportacion", command=lambda:self.tomarConfirmacionExportacion("No"))
        self.lbl_preguntanegativa_export.place(relx=0.65, rely=0.65, anchor="center", y=27)

    def tomarConfirmacionExportacion(self,respuesta):
        if respuesta == "Si":
            self.gestor.confirmarExportacion()
        else:
            fin_cu=self.gestor.finCu(True)
            if fin_cu:
                # Termina el caso de uso
